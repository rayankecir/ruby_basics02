...
def how_much_twitter
    puts "il y a: #{twitter.length} handle twitter"
    return twitter.length
end

# Quelle est le handle le plus court de cette liste ?
def min_twitter
    shortest_twitter = twitter.min_by { |twitter| twitter.length }
    puts shortest_twitter
end

# Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
def find_handles_with_5_characters(twitter)
    result = []
 twitter.each do |twitter|
    twitter_without_at = twitter.delete('@')
    if twitter_without_at.length == 5
        result << twitter
    end
end
...
def twitter_majuscules(twitter)
    twitter_maj = []
    twitter.each do |twitter|
        if twitter[0] == twitter[0].upcase
            twitter_maj << twitter
        end
    end
...
def trier_twitter(twitter)
    twitter.sort
end

# Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)
def trier_taille_twitter(twitter)
    twitter.sort_by! { |twitter| twitter.length }
end

# Quelle est la position dans l'array de la personne @epenser ?
def epenser_position(twitter)
    handles.index('@epenser') + 1
end
